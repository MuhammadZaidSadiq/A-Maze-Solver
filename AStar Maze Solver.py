...
class MazeSolver:
    ...
    def solve_astar(self, callback=None):
        def heuristic(a, b):
            ...
            for dx, dy in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                ...
                if (0 <= nx < self.width and 0 <= ny < self.height and 
                    ...
                    if tentative_g < g_score.get(neighbor, float('inf')):
                        ...
                        f_score[neighbor] = tentative_g + heuristic(neighbor, self.end)
...
class MazeGUI:
    def __init__(self, root, width=31, height=21):
        ...
        self.description = tk.Label(
            self.control_frame,
            text="A* combines Dijkstra's algorithm and greedy best-first search using a heuristic. It finds the shortest path while exploring fewer nodes than BFS.",
            wraplength=200,
            justify=tk.LEFT
        )
    ...
    def solve_maze(self):
        ...
        self.clear_solution()
        self.status_var.set("Solving with A*...")
        self.root.update()
        
        start_time = time.time()
        solution = self.solver.solve_astar(callback=self.update_cell)
        end_time = time.time()
